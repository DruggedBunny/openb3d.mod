...
{ 
	...
	if (gl_TexCoord[0].x<(vx_offset-0.01)) {
		...
		tc = texture2D(sceneTex, uv).rgb * 0.2270270270; //weight[0]
		//for (int i=1; i<3; i++) 
		tc += texture2D(sceneTex, uv + vec2(0.0, 1.3846153846)/rt_h).rgb * 0.3162162162;
		tc += texture2D(sceneTex, uv - vec2(0.0, 1.3846153846)/rt_h).rgb * 0.3162162162;
		tc += texture2D(sceneTex, uv + vec2(0.0, 3.2307692308)/rt_h).rgb * 0.0702702703;
		tc += texture2D(sceneTex, uv - vec2(0.0, 3.2307692308)/rt_h).rgb * 0.0702702703;
	}
